...
class Directory implements DirectoryContract
{
	...
	public function __construct(Website $website)
	{
		...
		$path = config('multitenant.tenant-directory') ? config('multitenant.tenant-directory') : storage_path('multitenant');
		
		if ($this->website->isDirty('identifier')) {
			$this->old_path = sprintf('%s/%s/',
				$path,
